...
func SetupRoutes() *mux.Router {
	...
	leccionHandler := handlers.NewLeccionHandler()
	inscripcionHandler := handlers.NewInscripcionHandler()
	...
	// --- Lecciones ---
	// Rutas para instructores
	api.HandleFunc("/cursos/{cursoId}/lecciones", middleware.RoleMiddleware("instructor", leccionHandler.Create)).Methods("POST")
	api.HandleFunc("/cursos/{cursoId}/lecciones/{leccionId}", middleware.RoleMiddleware("instructor", leccionHandler.Update)).Methods("PUT")
	api.HandleFunc("/cursos/{cursoId}/lecciones/{leccionId}", middleware.RoleMiddleware("instructor", leccionHandler.Delete)).Methods("DELETE")

	// Rutas disponibles para todos los usuarios autenticados
	api.HandleFunc("/cursos/{cursoId}/lecciones", middleware.AuthMiddleware(leccionHandler.GetByCurso)).Methods("GET")
	api.HandleFunc("/cursos/{cursoId}/lecciones/{leccionId}", middleware.AuthMiddleware(leccionHandler.GetByID)).Methods("GET")

	// --- Inscripciones ---
	// Rutas para alumnos
	api.HandleFunc("/inscripciones", middleware.RoleMiddleware("alumno", inscripcionHandler.Inscribir)).Methods("POST")
	api.HandleFunc("/inscripciones/mis-inscripciones", middleware.RoleMiddleware("alumno", inscripcionHandler.GetMisInscripciones)).Methods("GET")
	api.HandleFunc("/inscripciones/{id}/progreso", middleware.RoleMiddleware("alumno", inscripcionHandler.MarcarLeccion)).Methods("POST")

	// Rutas disponibles para todos los usuarios autenticados
	api.HandleFunc("/inscripciones/{id}", middleware.AuthMiddleware(inscripcionHandler.GetByID)).Methods("GET")
	api.HandleFunc("/inscripciones/{id}/progreso", middleware.AuthMiddleware(inscripcionHandler.GetProgreso)).Methods("GET")
